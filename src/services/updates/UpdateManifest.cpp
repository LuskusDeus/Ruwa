    ...
    const QByteArray& json, qint64 maximumArchiveBytes, QString* errorMessage)
{
    ...
    for (const QJsonValue& fileValue : files) {
        ...
        if (!isSafeRelativePath(file.sourcePath) || !isSafeRelativePath(file.targetPath) || !fileSize
            || *fileSize > maximumArchiveBytes || totalFileBytes > maximumArchiveBytes - *fileSize
            || !isSha256(fileHash)
        ...
        containsExecutable
            = containsExecutable || file.targetPath.compare(QStringLiteral("Ruwa.exe"), Qt::CaseInsensitive) == 0;
