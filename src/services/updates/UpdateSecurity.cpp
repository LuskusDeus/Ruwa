    ...
    const QString& pinnedCertificateSha256, QString* errorMessage)
{
    ...
    const QByteArray certificateBytes(reinterpret_cast<const char*>(signerCertificate->pbCertEncoded),
    ...
    if (actualFingerprint.compare(normalizedFingerprint(pinnedCertificateSha256), Qt::CaseInsensitive)
        != 0) {
        setError(errorMessage, QStringLiteral("Update manifest was signed by an unknown publisher"));
        return false;
    }
    return true;
#else
