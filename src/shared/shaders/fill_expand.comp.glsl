...
layout(std430, binding = 1) readonly buffer CurrentFrontierBuffer {
    uint currentFrontier[];
};

layout(std430, binding = 2) writeonly buffer NextFrontierBuffer {
    uint nextFrontier[];
};
...
void enqueueIfEligible(ivec2 pixelCoord) {
    ...
    uint writeIndex = atomicAdd(nextCount, 1u);
    nextFrontier[writeIndex] = packedIndex;
...
void main() {
    ...
    uint packedIndex = currentFrontier[index];
    uint width = uint(uCanvasSize.x);
    ivec2 pixelCoord = ivec2(int(packedIndex % width), int(packedIndex / width));

    for (int i = 0; i < 4; ++i) {
        enqueueIfEligible(pixelCoord + kNeighborOffsets[i]);
    }
}
