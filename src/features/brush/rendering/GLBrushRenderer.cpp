...
static const QString kProceduralTextureFrag = QStringLiteral(
    ...
    "uniform float uDotsSpacing;\n"
    "uniform float uDotsSize;\n"
    "uniform float uDotsJitter;\n"
    "uniform float uLinesSpacing;\n"
    "uniform float uLinesThickness;\n"
    "uniform float uLinesAngle;\n"
    "uniform float uCheckerSize;\n"
    "uniform float uCheckerSoftness;\n"
    "uniform float uCheckerRotation;\n"
    "in vec2 fragTexCoord;\n"
    "out float outValue;\n"
    ...
    "    float midWeight = 0.15 + uPencilDetail * 0.2;\n"
    "    float highWeight = uPencilDetail * 0.2;\n"
    "    float streakWeight = uPencilStreakStrength * 0.5;\n"
    "    float weightSum = 0.5 + midWeight + highWeight + streakWeight;\n"
    "    return clamp((n0 * 0.5 + n1 * midWeight + n2 * highWeight\n"
    "                    + streak * streakWeight) / weightSum, 0.0, 1.0);\n"
    "}\n"
    "uint fractalSeed(int octave) {\n"
    "    if (octave == 0) return 0xB2C4D1u;\n"
    "    if (octave == 1) return 0x8E3A5Fu;\n"
    "    if (octave == 2) return 0xD7F21Au;\n"
    "    if (octave == 3) return 0x4CA591u;\n"
    "    return 0xE19B37u;\n"
    "}\n"
    "float noiseGrain(float wx, float wy, float scale) {\n"
    "    float frequency = 0.025 * scale;\n"
    "    float amplitude = 1.0;\n"
    "    float value = 0.0;\n"
    "    float weight = 0.0;\n"
    "    for (int octave = 0; octave < 5; ++octave) {\n"
    "        if (octave >= uNoiseOctaves) break;\n"
    "        value += valueNoise(wx * frequency, wy * frequency, fractalSeed(octave))\n"
    "               * amplitude;\n"
    "        weight += amplitude;\n"
    "        frequency *= 2.35;\n"
    "        amplitude *= uNoiseRoughness;\n"
    "    }\n"
    "    return clamp(value / max(weight, 0.0001), 0.0, 1.0);\n"
    "}\n"
    "uint perlinSeed(int octave) {\n"
    "    if (octave == 0) return 0x5A3C91u;\n"
    "    if (octave == 1) return 0x7B2D41u;\n"
    "    if (octave == 2) return 0xC4E137u;\n"
    "    if (octave == 3) return 0x326FA9u;\n"
    "    return 0xD9155Bu;\n"
    "}\n"
    "float perlinGrain(float wx, float wy, float scale) {\n"
    "    float frequency = 0.015 * scale;\n"
    "    float amplitude = 1.0;\n"
    "    float value = 0.0;\n"
    "    float weight = 0.0;\n"
    "    for (int octave = 0; octave < 5; ++octave) {\n"
    "        if (octave >= uPerlinOctaves) break;\n"
    "        float signedNoise = gradientNoise(\n"
    "            wx * frequency, wy * frequency, perlinSeed(octave)) * 2.0 - 1.0;\n"
    "        value += signedNoise * amplitude;\n"
    "        weight += amplitude;\n"
    "        frequency *= 2.0;\n"
    "        amplitude *= uPerlinPersistence;\n"
    "    }\n"
    "    return clamp(0.5 + 0.5 * value / max(weight, 0.0001), 0.0, 1.0);\n"
    "}\n"
    "float dotsGrain(float wx, float wy, float scale) {\n"
    "    float period = uDotsSpacing / scale;\n"
    "    vec2 grid = vec2(wx, wy) / period;\n"
    "    ivec2 cell = ivec2(floor(grid));\n"
    "    float jitterAmount = uDotsJitter * 0.35;\n"
    "    vec2 jitter = vec2(hash01(cell.x, cell.y, 0xD07A31u),\n"
    "                       hash01(cell.x, cell.y, 0x91C5E7u)) * 2.0 - 1.0;\n"
    "    vec2 local = fract(grid) - vec2(0.5) - jitter * jitterAmount;\n"
    "    float radius = uDotsSize * 0.5;\n"
    "    float feather = clamp(1.0 / period, 0.0125, 0.12);\n"
    "    return 1.0 - smoothstep(radius - feather, radius + feather, length(local));\n"
    "}\n"
    "float linesGrain(float wx, float wy, float scale) {\n"
    "    float period = uLinesSpacing / scale;\n"
    "    float angle = radians(uLinesAngle);\n"
    "    float coordinate = (wx * cos(angle) + wy * sin(angle)) / period;\n"
    "    float phase = fract(coordinate);\n"
    "    float distance = min(phase, 1.0 - phase);\n"
    "    float halfWidth = uLinesThickness * 0.5;\n"
    "    float feather = clamp(1.0 / period, 0.01, 0.12);\n"
    "    return 1.0 - smoothstep(halfWidth - feather, halfWidth + feather, distance);\n"
    "}\n"
    "float checkerGrain(float wx, float wy, float scale) {\n"
    "    float cellSize = uCheckerSize / scale;\n"
    "    float angle = radians(uCheckerRotation);\n"
    "    float cosine = cos(angle);\n"
    "    float sine = sin(angle);\n"
    "    vec2 grid = vec2(wx * cosine + wy * sine, -wx * sine + wy * cosine)\n"
    "              / cellSize;\n"
    "    ivec2 cell = ivec2(floor(grid));\n"
    "    float checker = ((cell.x + cell.y) & 1) == 0 ? 1.0 : 0.0;\n"
    "    if (uCheckerSoftness <= 0.0001) return checker;\n"
    "    vec2 local = fract(grid);\n"
    "    float edgeDistance = min(min(local.x, 1.0 - local.x),\n"
    "                             min(local.y, 1.0 - local.y));\n"
    "    float interior = smoothstep(0.0, uCheckerSoftness, edgeDistance);\n"
    "    return 0.5 + (checker - 0.5) * interior;\n"
    "}\n"
    ...
    "        g = dotsGrain(wx, wy, uScale);\n"
    "    } else if (uTextureType == 4) {\n"
    "        g = linesGrain(wx, wy, uScale);\n"
    "    } else if (uTextureType == 5) {\n"
    "        g = checkerGrain(wx, wy, uScale);\n"
    "    } else {\n"
...
GLuint GLBrushRenderer::ensureProceduralTextureTile(const TileKey& key, const TileBrush& brush)
{
    ...
    m_proceduralTextureProgram->setUniform(
        "uPencilStreakStrength", brush.texturePencilStreakStrength());
    m_proceduralTextureProgram->setUniform(
        "uNoiseOctaves", static_cast<int>(std::lround(brush.textureNoiseOctaves())));
    ...
    m_proceduralTextureProgram->setUniform("uLinesSpacing", brush.textureLinesSpacing());
