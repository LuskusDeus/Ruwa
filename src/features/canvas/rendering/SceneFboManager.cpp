...
void SceneFboManager::copyRegionFromDefaultFbo(
    QOpenGLFunctions_4_5_Core* gl, GLint defaultFbo, int x, int y, int width, int height) const
{
    ...
    gl->glBlitFramebuffer(
        x, y, x + width, y + height, x, y, x + width, y + height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
    gl->glBindFramebuffer(GL_READ_FRAMEBUFFER, 0);
    gl->glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
    gl->glBindFramebuffer(GL_FRAMEBUFFER, static_cast<GLuint>(defaultFbo));
}
