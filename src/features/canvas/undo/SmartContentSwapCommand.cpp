...
{
    // The two grids trade places on every undo/redo, so the footprint this
    // command is responsible for is the larger of them.
    const qint64 appliedMemory = m_content ? gridMemorySize(m_content->grid.get()) : qint64 { 0 };
    m_stateMemoryUpperBound = std::max(gridMemorySize(m_inactiveState.grid.get()), appliedMemory);
}
