...
TEST_CASE("Chromatic LUT cell does not switch to virtual-black metamers",
    "[pigment][lut][regression]")
{
    // The eight 33^3-LUT nodes surrounding #503850 previously alternated
    // between about 0% and 91% virtual black. Any interpolated color inside
    // that cell could therefore acquire black even though both source paints
    // (#F85000 and #5010F8) contain none.
    constexpr std::array<PigmentModel::Srgb, 8> cellCorners {
        PigmentModel::Srgb { 10.0f / 32.0f, 7.0f / 32.0f, 10.0f / 32.0f },
        PigmentModel::Srgb { 10.0f / 32.0f, 7.0f / 32.0f, 11.0f / 32.0f },
        PigmentModel::Srgb { 10.0f / 32.0f, 8.0f / 32.0f, 10.0f / 32.0f },
        PigmentModel::Srgb { 10.0f / 32.0f, 8.0f / 32.0f, 11.0f / 32.0f },
        PigmentModel::Srgb { 11.0f / 32.0f, 7.0f / 32.0f, 10.0f / 32.0f },
        PigmentModel::Srgb { 11.0f / 32.0f, 7.0f / 32.0f, 11.0f / 32.0f },
        PigmentModel::Srgb { 11.0f / 32.0f, 8.0f / 32.0f, 10.0f / 32.0f },
        PigmentModel::Srgb { 11.0f / 32.0f, 8.0f / 32.0f, 11.0f / 32.0f },
    };

    for (const auto& color : cellCorners) {
        const auto latent = PigmentModel::encode(color);
        CAPTURE(color.r, color.g, color.b, latent.pigments[1]);
        REQUIRE(latent.pigments[1] < 0.10f);
    }

    const auto black = PigmentModel::encode({ 0.0f, 0.0f, 0.0f });
    REQUIRE(black.pigments[1] > 0.99f);
}
