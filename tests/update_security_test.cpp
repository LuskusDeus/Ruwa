...
TEST_CASE("trusted certificate fingerprints are strict SHA-256", "[updates][security]")
{
    ...
    CHECK(isValidCertificateSha256(QStringLiteral(
...
TEST_CASE("detached update signature accepts only the pinned publisher", "[updates][security]")
{
    ...
    const QByteArray signature = QByteArray::fromBase64(QByteArrayLiteral(
        "MIIEYAYJKoZIhvcNAQcCoIIEUTCCBE0CAQExDzANBglghkgBZQMEAgEFADALBgkqhkiG9w0B"
    ...
    QString error;
    CHECK(ruwa::services::verifyDetachedUpdateSignature(
        content, signature, fingerprint, &error));
